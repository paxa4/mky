...
export default function ArticlesModule({ articles, saveArticle, deleteArticle, changeArticleStatus }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 2, background: "#F1F5F9", borderRadius: 10, padding: 4, marginBottom: 28, width: "fit-content" }}>
        {SUB_TABS.map(({ id, label, icon }) => (
          <button
            key={id}
            type="button"
            onClick={() => { setSubTab(id); setEditing(null); }}
            style={{
              padding: "10px 18px",
              borderRadius: 8,
              fontWeight: 600,
          ...
          >
            ...
            {id === "articles" && (
              <span style={{ marginLeft: 4, fontSize: 11, background: subTab === id ? "#EFF6FF" : "#E2E8F0", borderRadius: 10, padding: "1px 7px", color: subTab === id ? "#1D4ED8" : "#64748B" }}>
      ...
      {subTab === "categories" && <CategoriesManager store={store} />}
      {subTab === "tags" && <TagsManager store={store} />}
    </div>
  );
}
