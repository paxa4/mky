...
function Tag({ children, bg = "#EFF6FF", color = "#1D4ED8" }) {
  return <span style={{ display: "inline-flex", padding: "3px 10px", borderRadius: 20, background: bg, color, fontSize: 12, fontWeight: 600 }}>{children}</span>;
}
...
// УДАЛЕНО: Вкладка "Уведомления" из массива TABS
const TABS = [
...
export default function ProfilePage({ user, onBack, onLogout, userArticles = [] }) {
  ...
  const roleName = user.role?.role_name || (user.role_id === 1 ? "admin" : user.role_id === 2 ? "methodist" : "teacher");

  return (
    <div style={{ minHeight: "100vh", background: "#F8FAFC", fontFamily: "'PT Sans', system-ui, sans-serif" }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        .ptab { padding: 9px 14px; border-radius: 10px; border: none; background: none; cursor: pointer; font-size: 13px; font-weight: 500; color: #64748B; display: flex; align-items: center; gap: 7px; font-family: inherit; transition: background 0.15s, color 0.15s; white-space: nowrap; width: 100%; text-align: left; }
        .ptab:hover  { background: #F1F5F9; color: #334155; }
        .ptab.active { background: #EFF6FF; color: #1D4ED8; font-weight: 700; }
        .pinput { width: 100%; padding: 10px 13px; font-size: 14px; border: 1.5px solid #E2E8F0; border-radius: 10px; outline: none; font-family: inherit; background: #F8FAFC; transition: border-color 0.2s; }
        .pinput:focus { border-color: #93C5FD; background: #fff; }
        @media (max-width: 768px) { .playout { flex-direction: column !important; } .psidebar { width: 100% !important; } .ptabs-col { flex-direction: row !important; overflow-x: auto; gap: 4px !important; } }
      `}</style>

      {/* Шапка */}
      <div style={{ background: "#fff", borderBottom: "1px solid #F1F5F9", padding: "0 24px", height: 58, display: "flex", alignItems: "center", gap: 12, boxShadow: "0 1px 8px rgba(0,0,0,0.04)" }}>
        <button onClick={onBack} style={{ background: "none", border: "1px solid #E2E8F0", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#64748B", padding: "6px 14px", borderRadius: 9, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5 }}>
          ← На главную
        </button>
        <span style={{ fontSize: 15, fontWeight: 700, color: "#0F172A" }}>Личный кабинет</span>
        <div style={{ flex: 1 }} />
        {/* УДАЛЕНО: Счетчик уведомлений в шапке */}
        {onLogout && (
          <button onClick={onLogout} style={{ background: "none", border: "1px solid #E2E8F0", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#EF4444", padding: "6px 14px", borderRadius: 9, fontFamily: "inherit" }}>
            Выйти
          </button>
        )}
      </div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", gap: 22, flexWrap: "wrap" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
              <Tag bg="rgba(255,255,255,0.18)" color="#fff">{ROLE_LABELS[roleName] || roleName}</Tag>
              {user.position     && <Tag bg="rgba(255,255,255,0.18)" color="#fff">{user.position}</Tag>}
              {user.qualification && <Tag bg="rgba(255,255,255,0.18)" color="#fff">{user.qualification}</Tag>}
              <Tag bg="rgba(255,255,255,0.12)" color="rgba(255,255,255,0.8)">
      ...
      {/* Контент */}
      <div className="playout" style={{ maxWidth: 1100, margin: "24px auto", padding: "0 24px", display: "flex", gap: 20, alignItems: "flex-start" }}>
        ...
        <div className="psidebar" style={{ width: 220, flexShrink: 0 }}>
          ...
          {daysLeft !== null && (
            <Card style={{ marginTop: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Аттестация</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#0F172A" }}>
                {new Date(user.nextAttestationDate).toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" })}
              </div>
              <div style={{ marginTop: 10, padding: "8px 10px", borderRadius: 9, background: daysLeft < 365 ? "#FFFBEB" : "#ECFDF5" }}>
            ...
            <>
              <Card title="Данные аккаунта" icon="👤">
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10 }}>
                  <button onClick={() => setEditMode(v => !v)} style={{ padding: "6px 14px", borderRadius: 8, border: "1.5px solid #BFDBFE", background: "#EFF6FF", color: "#1D4ED8", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                ...
                {editMode ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {[
                      ...
                      <div key={f.key}>
                        <label style={{ fontSize: 11, fontWeight: 700, color: "#64748B", display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.05em" }}>{f.label}</label>
                        <input className="pinput" value={form[f.key]} onChange={e => setForm(v => ({ ...v, [f.key]: e.target.value }))} />
                      </div>
                    ))}
                    <button onClick={() => setEditMode(false)} style={{ padding: "10px", borderRadius: 10, border: "none", background: "#1D4ED8", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>
                      Сохранить
                    </button>
                  </div>
                ) : (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
                {[
                  ...
                  { label: "Часов обучения",  value: (user.certificates || []).reduce((s, c) => s + c.hours, 0), icon: "⏱️", bg: "#F0FDF4", tc: "#059669" },
                ].map(st => (
                  <div key={st.label} style={{ background: st.bg, borderRadius: 14, padding: "18px 16px", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 24, fontWeight: 800, color: st.tc, lineHeight: 1.2, marginTop: 4 }}>{st.value}</div>
          ...
          {activeTab === "articles" && (
            <Card title="Мои статьи" icon="📝">
              {userArticles.length === 0 ? (
                ...
                const statusStyle = a.status?.name === "published" ? { bg: "#ECFDF5", color: "#059669" }
                  : a.status?.name === "draft" ? { bg: "#FFFBEB", color: "#D97706" }
                ...
                return (
                  <div key={a.id} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "14px 0", borderBottom: "1px solid #F8FAFC" }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#94A3B8" }}>/{a.slug} · обновлено {a.updated_at?.slice(0, 10)}</div>
                    </div>
                    <Tag bg={statusStyle.bg} color={statusStyle.color}>
                      {a.status?.name === "published" ? "Опубликована" : a.status?.name === "draft" ? "Черновик" : "Архив"}
                    </Tag>
                  </div>
                );
              })}
                ...
                <div key={cert.id} style={{ display: "flex", gap: 14, alignItems: "flex-start", padding: "14px 0", borderBottom: "1px solid #F8FAFC" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <Tag bg="#F1F5F9" color="#475569">{new Date(cert.date).toLocaleDateString("ru-RU", { month: "long", year: "numeric" })}</Tag>
                    </div>
                  </div>
                  <button style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#fff", fontSize: 12, fontWeight: 600, color: "#1D4ED8", cursor: "pointer", whiteSpace: "nowrap", fontFamily: "inherit" }}>
                    📄 Скачать
                  </button>
                </div>
              ))}
              {(user.certificates || []).length === 0 && (
                <p style={{ color: "#94A3B8", fontSize: 14, textAlign: "center", padding: "32px 0" }}>Курсов пока нет</p>
              )}
            </Card>
          )}
          ...
          {activeTab === "achievements" && (
            <Card title="Конкурсы и достижения" icon="🏆">
              {(user.achievements || []).map(ach => {
                ...
                return (
                  <div key={ach.id} style={{ display: "flex", gap: 14, alignItems: "center", padding: "14px 0", borderBottom: "1px solid #F8FAFC" }}>
                    <div style={{ width: 42, height: 42, borderRadius: 12, background: "#FFFBEB", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>🏆</div>
                    <div style={{ flex: 1 }}>
              ...
              {(user.achievements || []).length === 0 && (
                <p style={{ color: "#94A3B8", fontSize: 14, textAlign: "center", padding: "32px 0" }}>Достижений пока нет</p>
              )}
            </Card>
          )}
            ...
            <Card title="Безопасность" icon="🔒">
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {["Текущий пароль", "Новый пароль", "Повторите новый пароль"].map(l => (
                  <div key={l}>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "#64748B", display: "block", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.05em" }}>{l}</label>
                ...
                <p style={{ fontSize: 12, color: "#94A3B8", margin: 0 }}>
                  Поле password_hash в таблице users обновляется через хэширование bcrypt на сервере.
                </p>
              </div>
            </Card>
          )}
