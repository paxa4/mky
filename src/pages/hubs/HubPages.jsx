...
function HubStyles() {
  return (
    <style>{`
      ...
      .hub-main { flex: 1; }
      .hub-shell { width: min(1180px, calc(100% - 28px)); margin: 0 auto; padding: 34px 0 68px; }
      .hub-breadcrumb { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 18px; color: #64748b; font-size: 14px; font-weight: 800; align-items: center; }
      .hub-breadcrumb a { color: #1e3a8a; text-decoration: none; }
      ...
      .hub-grid, .hub-news-grid, .methodika-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 260px), 360px)); justify-content: start; gap: 12px; margin-top: 18px; }
      ...
      .hub-empty { border: 1px solid #dbe6f5; border-radius: 8px; background: #fff; color: #475569; padding: 18px; font-weight: 750; line-height: 1.55; margin-top: 18px; }
      ...
      @media (min-width: 720px) {
        .hub-shell { width: min(1180px, calc(100% - 44px)); padding-top: 46px; }
      }
      @media (max-width: 520px) {
        .hub-shell { width: min(100% - 24px, 1180px); }
      }
    `}</style>
  );
}
  ...
  return (
    <HubShell {...props}>
      <div className="hub-shell">
        <nav className="hub-breadcrumb" aria-label="Навигация">
    ...
    <HubShell {...props}>
      <div className="hub-shell">
        <nav className="hub-breadcrumb" aria-label="Навигация">
          <Link to="/">Главная</Link>
          <span>→</span>
          <span>Методическое пространство</span>
        </nav>
        <section className="hub-hero">
          <span className="hub-eyebrow">Методическое пространство</span>
          <h1>Предметные кабинеты</h1>
          <p>Все предметные направления равноправны: выберите кабинет, чтобы открыть материалы, рекомендации и практики.</p>
        </section>
