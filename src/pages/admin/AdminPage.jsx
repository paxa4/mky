...
import { useAdminMeta, generateSlug } from "../../features/admin/adminStore.js";
...
function MultiSelect({ options, value, onChange, onCreate, placeholder }) {
  ...
  const filtered = options.filter(
    o => o.name.toLowerCase().includes(input.toLowerCase()) && !value.includes(o.id)
  );
  return (
            ...
            <span key={id} style={{ display: "inline-flex", alignItems: "center", gap: 4,
              ...
              <button onClick={e => { e.stopPropagation(); onChange(value.filter(v => v !== id)); }}
                style={{ background: "none", border: "none", cursor: "pointer",
                  color: "#1D4ED8", padding: 0, lineHeight: 1, fontSize: 14 }}>×</button>
            </span>
          ) : null;
        })}
        <input value={input}
          ...
          style={{ border: "none", outline: "none", background: "transparent",
            fontSize: 13, color: "#0F172A", minWidth: 80, flex: 1, fontFamily: "inherit" }} />
      </div>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0,
          ...
          {onCreate && input.trim() && !options.find(o => o.name.toLowerCase() === input.toLowerCase()) && (
            <div onMouseDown={() => { const id = onCreate(input.trim()); onChange([...value, id]); setInput(""); }}
              style={{ padding: "9px 14px", fontSize: 13, color: "#1D4ED8", fontWeight: 600,
...
// ─── Форма статьи ─────────────────────────────────────────────────────────────
function ArticleForm({ article, categories, tags, addTag, onSave, onCancel, saveArticle }) {
    ...
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        <button onClick={onCancel}
          style={{ padding: "8px 14px", borderRadius: 9, border: "1px solid #E2E8F0",
            background: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600,
            color: "#64748B", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}>
          ← Назад
        </button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20, alignItems: "start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #F1F5F9", padding: "20px 24px" }}>
            ...
            <input value={form.title} onChange={e => handleTitle(e.target.value)}
              ...
              style={{ width: "100%", fontSize: 20, fontWeight: 700, border: "none",
                outline: "none", color: "#0F172A", fontFamily: "inherit", background: "transparent" }} />
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10,
              paddingTop: 10, borderTop: "1px solid #F1F5F9" }}>
              <span style={{ fontSize: 12, color: "#94A3B8" }}>Slug:</span>
              <input value={form.slug}
                onChange={e => { set("slug", e.target.value); set("_slugEdited", true); }}
                style={{ flex: 1, fontSize: 12, border: "none", outline: "none",
                  color: "#64748B", fontFamily: "inherit", background: "transparent" }} />
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {/* Status — с подсказкой про публикацию */}
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #F1F5F9", padding: "16px 18px" }}>
            ...
            {["published", "draft", "archive"].map(s => (
              <label key={s} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", cursor: "pointer" }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 500, color: STATUS_COLORS[s]?.color }}>
                  {STATUS_LABELS[s]}
                </span>
              </label>
            ))}
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #F1F5F9", padding: "16px 18px" }}>
            ...
            <MultiSelect options={categories} value={form.categories}
              onChange={v => set("categories", v)} placeholder="Выберите категории..." />
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #F1F5F9", padding: "16px 18px" }}>
            ...
            <MultiSelect options={tags} value={form.tags}
              onChange={v => set("tags", v)} onCreate={addTag}
              placeholder="Теги (можно создать новый)..." />
          </div>
        </div>
      </div>
    </div>
...
// ─── Таблица статей ───────────────────────────────────────────────────────────
function ArticlesList({ articles, categories, tags, onNew, onEdit, deleteArticle, changeArticleStatus }) {
  ...
  const sel = { padding: "7px 10px", borderRadius: 8, border: "1.5px solid #E2E8F0",
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="🔍 Поиск по заголовку..." style={{ ...sel, flex: 1, minWidth: 200 }} />
        ...
        <select value={filterAuth} onChange={e => setFilterAuth(e.target.value)} style={sel}>
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={7} style={{ padding: "32px", textAlign: "center",
                color: "#94A3B8", fontSize: 14 }}>Статьи не найдены</td></tr>
            )}
            {filtered.map(a => {
              ...
              return (
                <tr key={a.id} style={{ borderBottom: "1px solid #F8FAFC" }}
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {a.categories.map(cid => {
                        ...
                        return c ? (
                          <span key={cid} style={{ fontSize: 11, padding: "2px 7px",
                            background: "#F1F5F9", borderRadius: 5, color: "#475569" }}>
                            {c.name}
                          </span>
                        ) : null;
                      })}
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    <span style={{ fontSize: 12, fontWeight: 600, padding: "3px 9px",
                      borderRadius: 6, ...sc }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <select value={a.status}
                        ...
                        style={{ padding: "5px 8px", borderRadius: 7, border: "1px solid #E2E8F0",
                          ...
                          color: "#475569", fontFamily: "inherit", outline: "none" }}>
                        {Object.entries(STATUS_LABELS).map(([v, l]) => (
                          <option key={v} value={v}>{l}</option>
                        ))}
                      </select>
                      <button onClick={() => {
                        if (window.confirm(`Удалить «${a.title}»?`)) deleteArticle(a.id);
                      }}
...
// ─── Категории ────────────────────────────────────────────────────────────────
function CategoriesManager({ articles, categories, addCategory, renameCategory, deleteCategory }) {
  const [newName, setNewName] = useState("");
  const [editing, setEditing] = useState(null);
  const usageCount = id => articles.filter(a => a.categories.includes(id)).length;
    ...
    <div style={{ maxWidth: 560 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <input value={newName} onChange={e => setNewName(e.target.value)}
          onKeyDown={e => { if (e.key === "Enter" && newName.trim()) { addCategory(newName.trim()); setNewName(""); } }}
          placeholder="Название новой категории"
          style={{ flex: 1, padding: "9px 14px", border: "1.5px solid #E2E8F0", borderRadius: 10,
            fontSize: 14, fontFamily: "inherit", outline: "none", background: "#F8FAFC" }} />
        <button onClick={() => { if (newName.trim()) { addCategory(newName.trim()); setNewName(""); } }}
          style={{ padding: "9px 18px", borderRadius: 10, border: "none", background: "#1D4ED8",
            ...
            <div key={cat.id} style={{ display: "flex", alignItems: "center", gap: 10,
              ...
              {isEdit ? (
                <input value={editing.name}
                  ...
                  onKeyDown={e => {
                    if (e.key === "Enter") { renameCategory(cat.id, editing.name); setEditing(null); }
                    if (e.key === "Escape") setEditing(null);
                  }}
                  autoFocus
                  style={{ flex: 1, padding: "6px 10px", border: "1.5px solid #93C5FD",
              ...
              {isEdit ? (
                <>
                  <button onClick={() => { renameCategory(cat.id, editing.name); setEditing(null); }}
                    style={{ padding: "5px 10px", borderRadius: 7, border: "none",
                      background: "#1D4ED8", color: "#fff", cursor: "pointer",
                      fontSize: 12, fontWeight: 600, fontFamily: "inherit" }}>✓</button>
                  <button onClick={() => setEditing(null)}
                    style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #E2E8F0",
                      background: "#fff", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>✕</button>
                </>
              ) : (
                <>
                  <button onClick={() => setEditing({ id: cat.id, name: cat.name })}
                    style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #E2E8F0",
                      background: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 600,
                      color: "#1D4ED8", fontFamily: "inherit" }}>Ред.</button>
                  <button onClick={() => {
                    ...
                    style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #FECACA",
                      ...
                      color: "#EF4444", fontFamily: "inherit" }}>Удалить</button>
                </>
              )}
            </div>
          );
...
// ─── Теги ─────────────────────────────────────────────────────────────────────
function TagsManager({ articles, tags, addTag, renameTag, deleteTag }) {
  const [newName, setNewName] = useState("");
  const [editing, setEditing] = useState(null);
  const usageCount = id => articles.filter(a => a.tags.includes(id)).length;
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <input value={newName} onChange={e => setNewName(e.target.value)}
          onKeyDown={e => { if (e.key === "Enter" && newName.trim()) { addTag(newName.trim()); setNewName(""); } }}
          placeholder="Название нового тега"
          style={{ flex: 1, padding: "9px 14px", border: "1.5px solid #E2E8F0", borderRadius: 10,
            fontSize: 14, fontFamily: "inherit", outline: "none", background: "#F8FAFC" }} />
        <button onClick={() => { if (newName.trim()) { addTag(newName.trim()); setNewName(""); } }}
          style={{ padding: "9px 18px", borderRadius: 10, border: "none", background: "#1D4ED8",
            color: "#fff", cursor: "pointer", fontSize: 14, fontWeight: 700, fontFamily: "inherit" }}>
          Добавить
        </button>
        ...
        {tags.map(tag => {
          ...
          return (
            <div key={tag.id} style={{ display: "flex", alignItems: "center", gap: 6,
              padding: "6px 10px", background: "#F1F5F9", borderRadius: 20,
              border: "1px solid #E2E8F0" }}>
              {isEdit ? (
                <input value={editing.name}
                  onChange={e => setEditing(v => ({ ...v, name: e.target.value }))}
                  onKeyDown={e => {
                    if (e.key === "Enter") { renameTag(tag.id, editing.name); setEditing(null); }
                    if (e.key === "Escape") setEditing(null);
                  }}
                  autoFocus
                  style={{ width: 100, padding: "2px 6px", border: "1px solid #93C5FD",
              ...
              {isEdit ? (
                <>
                  <button onClick={() => { renameTag(tag.id, editing.name); setEditing(null); }}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#059669", fontSize: 14, padding: 0 }}>✓</button>
                  <button onClick={() => setEditing(null)}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#94A3B8", fontSize: 14, padding: 0 }}>✕</button>
                </>
              ) : (
                <>
                  <button onClick={() => setEditing({ id: tag.id, name: tag.name })}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#1D4ED8", fontSize: 12, padding: 0 }}>✏️</button>
                  <button onClick={() => {
...
/**
 * AdminPage принимает пропсами:
 *   onBack              — вернуться на сайт
 *   articles            — массив всех статей из App.jsx
 *   saveArticle         — создать/обновить статью
 *   deleteArticle       — удалить
 *   changeArticleStatus — сменить статус
 *
 * Категории и теги — локальные (не нужны в ленте новостей, используются только здесь).
 */
export default function AdminPage({ onBack, articles, saveArticle, deleteArticle, changeArticleStatus }) {
  const meta = useAdminMeta(); // локальные категории и теги
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F1F5F9",
      ...
      {/* Header */}
      <div style={{ background: "#0F172A", padding: "0 24px", height: 56,
        ...
        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#1D4ED8",
          ...
          color: "#fff", fontSize: 14, fontWeight: 700 }}>А</div>
      </div>

      <div style={{ display: "flex", minHeight: "calc(100vh - 56px)" }}>
        {/* Sidebar */}
        <div style={{ width: 220, background: "#fff", borderRight: "1px solid #F1F5F9",
          padding: "20px 12px", flexShrink: 0 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#94A3B8",
            letterSpacing: "0.1em", textTransform: "uppercase",
            padding: "0 8px", marginBottom: 8 }}>Контент</div>
          {SECTIONS.map(s => (
            <button key={s.id}
            ...
            >
              ...
              {s.id === "articles" && (
                <span style={{ marginLeft: "auto", fontSize: 11,
                  background: "#E2E8F0", borderRadius: 10, padding: "1px 7px" }}>
                  {articles.length}
                </span>
              )}
            </button>
        ...
        <div style={{ flex: 1, padding: "28px 32px", overflow: "auto" }}>
          ...
          {section === "categories" && (
            <CategoriesManager
              articles={articles}
              categories={meta.categories}
              addCategory={meta.addCategory}
              renameCategory={meta.renameCategory}
          ...
          {section === "tags" && (
            <TagsManager
              ...
              tags={meta.tags}
              addTag={meta.addTag}
              renameTag={meta.renameTag}
              deleteTag={meta.deleteTag}
            />
          )}
        </div>
      </div>
